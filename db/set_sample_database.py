...
import db.reset_database as reset_database  # noqa: F401  (wipes + reseeds technicians, clears appointments)
from config import (
    BUSINESS_DAYS,
    BUSINESS_END_HOUR,
    BUSINESS_START_HOUR,
...
for row in rows:
    ...
    assert start_dt.minute in (0, 30) and start_dt.second == 0, (
        f"appointment {appt_id} does not start on the hour or half-hour: {start_time}"
    )
    assert end_dt.minute in (0, 30) and end_dt.second == 0, (
        f"appointment {appt_id} does not end on the hour or half-hour: {end_time}"
    )
